...
/**
 * Claim a gig for a musician (first to claim wins). Shared by the HTTP
 * claim endpoint and the inbound-SMS "YES" handler.
 */
function claimGig(gigId, musicianId) {
  const gig = db.prepare('SELECT * FROM gigs WHERE id = ?').get(gigId);
  if (!gig) return { error: 'not_found' };
  if (gig.status !== 'open') return { error: 'already_filled', gig };

  db.prepare(`
    UPDATE gigs SET status = 'filled', matched_musician_id = ?, filled_at = CURRENT_TIMESTAMP
    WHERE id = ?
  `).run(musicianId, gigId);
  ...
  return { gig, venue, musician, pricing };
}

module.exports = { findMatchingMusicians, calculatePricing, claimGig };
